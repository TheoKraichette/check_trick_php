...
class Trick
{
    ...
    public function getExpertTricks($co)
    {
        $query = "SELECT DISTINCT * FROM tricks t, difficulty d WHERE d.libelle_diff = 'expert' AND t.id_diff = d.id_diff GROUP BY t.nom_trick ORDER BY t.id_trick";
        $stmt = $co->prepare($query);
        $stmt->execute();
        $sel = $stmt->fetchAll();
